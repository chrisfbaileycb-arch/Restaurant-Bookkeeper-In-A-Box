...
export async function accountMap(ctx) {
  const r = await db.query('SELECT id, account_no, name, type FROM accounts WHERE active AND organization_id = $1', [ctx.orgId]);
  const map = new Map();
...
export async function accountBalances(ctx, from = '0000-01-01', to = '9999-12-31') {
  const r = await db.query(
    'SELECT a.account_no, a.name, a.type, a.qb_type, ' +
      ...
      'GROUP BY a.account_no, a.name, a.type, a.qb_type ORDER BY a.account_no',
