...
export async function profitLoss(ctx, from, to) {
  ...
  // Prime cost = COGS + labor (wages rollup + employer payroll taxes) as a
  // percent of revenue; the standard industry warning threshold is 65%.
  const laborCost = cents(expenses
    .filter((r) => r.name.startsWith('Wages') || r.name === 'Payroll Taxes')
  ...
  return {
    ...
    kpis: {
      ...
      laborCostPct: totalRevenue > 0 ? cents((laborCost / totalRevenue) * 100) : null,
      primeCostPct: totalRevenue > 0 ? cents((primeCost / totalRevenue) * 100) : null,
      primeCostStatus: totalRevenue > 0
        ? (cents((primeCost / totalRevenue) * 100) > 65 ? 'WARNING: prime cost above 65% of sales' : 'healthy')
        : null,
    },
  };
}
...
/** Pure variance math for two P&L results (period 2 relative to period 1). */
export function comparePeriods(p1, p2) {
  const delta = (a, b) => ({
    period1: a,
    period2: b,
    variance: cents(b - a),
    pctChange: a !== 0 ? cents(((b - a) / Math.abs(a)) * 100) : null,
  });
  return {
    revenue: delta(p1.totals.revenue, p2.totals.revenue),
    cogs: delta(p1.totals.cogs, p2.totals.cogs),
    grossProfit: delta(p1.totals.grossProfit, p2.totals.grossProfit),
    expenses: delta(p1.totals.expenses, p2.totals.expenses),
    netIncome: delta(p1.totals.netIncome, p2.totals.netIncome),
    primeCostPct: { period1: p1.kpis.primeCostPct, period2: p2.kpis.primeCostPct },
  };
}

/** Side-by-side P&L comparison of two date ranges. */
export async function comparativeStatements(ctx, p1From, p1To, p2From, p2To) {
  const p1 = await profitLoss(ctx, p1From, p1To);
  const p2 = await profitLoss(ctx, p2From, p2To);
  return {
    location: ctx.locationName,
    period1: { from: p1From, to: p1To, totals: p1.totals, kpis: p1.kpis },
    period2: { from: p2From, to: p2To, totals: p2.totals, kpis: p2.kpis },
    comparison: comparePeriods(p1, p2),
  };
}
